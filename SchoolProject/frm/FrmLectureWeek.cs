...
{
    public partial class FrmLectureWeek : BaseForm
    {
        public FrmLectureWeek()
        {
            InitializeComponent();
        }
        ...
        {
            int cnt = 0;
            foreach (DataModel.course p in lstCourses.SelectedItems)
            {
        ...
        List<int> GetCourseList()
        {
            ...
            foreach(DataModel.course crs in lstCourses.SelectedItems)
            {
                if (crs.sortshowInReport <= lstCourseIdentify.Count(a => a.courseid == crs.courseid))
                    lst.Remove(crs.courseid);
            }
            lst.Shuffle();
            return lst;
        }
        ...
        List<int> GetCountLectureByWeek(int lvlid)
        {
            List<int> lst = new List<int>();
            var obj = ctx.levels.FirstOrDefault(a => a.levelid == lvlid);
            int cntlecture = 0;
            while (cntlecture < obj.numbercourseterm1)
            {
                cntlecture++;
                lst.Add(cntlecture);
            }
        ...
        void GenerateCourse()
        {
            ...
            foreach(int l in lstlvl)
            {
                ...
                while (dyNumber < 7)
                {
                    ...
                    var lstlec = GetCountLectureByWeek(l);// LstLecture(dyNumber);
            ...
            lectureWeekBindingSource.DataSource = lstlw.ToList();
